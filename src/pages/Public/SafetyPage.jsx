...
const SafetyPage = () => {
  ...
  const loadContent = async () => {
    try {
      ...
      if (data && data.safetyPolicy) {
        setContent(data.safetyPolicy.content || '');
        setLastUpdated(data.safetyPolicy.lastUpdated || null);
      }
    } catch (err) {
      console.error('Error loading safety policy:', err);
      setError('Unable to load safety policy. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
          ...
          {!loading && !error && (
            <div className="content">
              {content ? (
                ...
                <p>Safety policy content is not available.</p>
              )}
            </div>
          )}
        </main>
